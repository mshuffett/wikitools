...
class APIRequest:
	...
	def query(self, querycontinue=True, iterations=0):
		...
		iterations - how many times to repeat the query if there is more data to retrieve
		...
		if 'query-continue' in data and querycontinue:
			data = self.__longQuery(data, iterations)
		return data
	
	def __longQuery(self, initialdata, iterations):
